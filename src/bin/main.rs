...
fn f_autogenerate_js(
    ...
    // Extract the data structure we're interested in, here assuming it's `a_o_name_synonym`
    let items = v["a_o_name_synonym"].as_array().unwrap();

    // Prepare to generate JavaScript code
    let mut js_variables = String::new();
    let mut js_array_items = vec![];

    for item in items {
        let name = item["s_name"].as_str().unwrap();
        let variable_name = format!("let s_name_o_input_sensor__{} = '{}';", name, name);
        js_variables.push_str(&variable_name);
        js_variables.push('\n');
        js_array_items.push(format!("s_name_o_input_sensor__{}", name));
    }
    ...
    let s_final_js_code = format!("{}\n{}\n{}", js_variables, js_array, export_block);

    let o_path = o_path_webserver_root.join("autogenerated.module.js");
    let s_path = o_path.as_os_str();
    let mut file = File::create(s_path).expect("Failed to create file");

    file.write_all(s_final_js_code.as_bytes()).expect("Failed to write to file");

    fs::set_permissions(s_path, fs::Permissions::from_mode(0o777)).unwrap();
...
fn f_b_write_s_json_o_config(s_str: &str) -> bool{
    let mut file = File::create(s_path_rel_file__config).expect("Failed to create file");

    file.write_all(s_str.as_bytes()).expect("Failed to write to file");


    fs::set_permissions(s_path_rel_file__config, fs::Permissions::from_mode(0o777)).unwrap();
...
fn f_s_json_o_config() -> String {
    match fs::read_to_string(s_path_rel_file__config) {
