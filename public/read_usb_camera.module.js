...
let s_name_file_config = 'o_config.json';
let s_name_file_config_default = 'o_config.default.json';

let o_config_default = JSON.parse((await f_o_ws_response({
  s_name_function: s_name_ws_action__f_s_read_text_file,
  s_path_rel: s_name_file_config_default
})).s_text);
 let o_state = {
  ...
  o_config_default: o_config_default,
  o_config: o_config_default,
...
let f_update_input_mapping_references = function(){
  o_state.o_input_action_mapping__next_action_layer = o_state.o_config.a_o_input_action_mapping__layer_switch.find(o=>
    o.o_input_action==o_mod_autogenerated.o_input_action__next_action_layer
  )
  o_state.o_input_action_mapping__prev_action_layer = o_state.o_config.a_o_input_action_mapping__layer_switch.find(o=>
    o.o_input_action==o_mod_autogenerated.o_input_action__prev_action_layer
  )
  ...
  o_state.o_input_action_mapping__keep_hold_down_toggle_layer2 = o_state.o_config.a_o_input_action_mapping__layer_switch.find(o=>
    o.o_input_action==o_mod_autogenerated.o_input_action__keep_hold_down_toggle_layer2
  )
  o_state.o_input_action_mapping__keep_hold_down_toggle_layer3 = o_state.o_config.a_o_input_action_mapping__layer_switch.find(o=>
    o.o_input_action==o_mod_autogenerated.o_input_action__keep_hold_down_toggle_layer3
  )
...
let f_try_start_webcam = async function(){
  ...
  try {
    ...
    .map(s=>{
      return {
        s_name: s, 
        ...o_s_name_o_capability[s]
      }
    })
    o_state.a_o_input_action.map(o=>{
      o.v_o_video_capability = o_state.a_o_video_capabilities.find(o2=>{
        let s_name_capability = o.s_name.split('_')[2];
        return o2.s_name == s_name_capability
      })
    })

      // Check if exposure is supported
    if (o_state.o_video_track.getCapabilities().exposureMode) {
      // Try to lock exposure (might not work on all devices)
      o_state.o_video_track.applyConstraints({ advanced: [{ exposureMode: 'manual' }] })
    ...
    if (o_state.o_video_track.getCapabilities().whiteBalanceMode) {
      // Try to lock exposure (might not work on all devices)
      o_state.o_video_track.applyConstraints({ advanced: [{ whiteBalanceMode: 'manual' }] })
        .then(() => console.log('Exposure locked (if supported)'))
        .catch(error => { 
          debugger
          console.error('Error locking exposure:', error)});
...
let f_r_ser_w_cli_o_config = async function(){
  ...
  let o_config = o_state.o_config_default;
  try{
    o_config = JSON.parse(o.s_text)
  }catch{
    console.log("no custom config exsits, copiying default to new custom file")
                                        ...
                                        a_o: [
                                          ...
                                          style: "display:flex; flex-direction:row;justify-content:space-between;align-items:center",
                                            a_o: [
                                              {
                                                class: 'keyboard_char',
                                                innerText: (o_state.o_config.o_input_action.s_name == o.s_name ) ? '?': o.s_name_char_keyboard.toUpperCase()
                                              },
                                              {
                                                innerHTML: "&nbsp;&nbsp;&nbsp;"
                                              },
                                              {
                                                class: f_s_class__from_s_name_font(v_o_input_font?.s_name_font),
                                                innerText: (o_state.o_config.o_input_action.s_name == o.s_name ) ? '?':  v_o_input_font?.s_char
                                              }
                                            ]
                                          }
                                          // f_o_jsh__keyicons(o.s_name_input)
