      ...
      if(n_ms_delta > n_ms_delta_max){
        if (
          ...
          if(!b_remap_input_action){
            ...
            // we first have to check if a 'layer switch' button is keep / held down
            let b_normal_layer = true; // if no layer button is pressed down this is the default 'layer' 


            let o_input_action__keep_hold_down_toggle_camera_control = o_state.o_config.a_o_input_action.find(
              o=>{return o.s_name == 'keep_hold_down_toggle_camera_control'}
            );
            let o_input_action__keep_hold_down_toggle_image_control = o_state.o_config.a_o_input_action.find(
              o=>{return o.s_name == 'keep_hold_down_toggle_image_control'}
            );
            
            let v_o_input_sensor_keep_hold_down_toggle_camera_control = o_state?.v_o_input_device?.a_o_input_sensor?.find(
              o=>{
                return o.s_name == o_input_action__keep_hold_down_toggle_camera_control?.s_name_input__controller
              }
            )
            let v_o_input_sensor_keep_hold_down_toggle_image_control = o_state?.v_o_input_device?.a_o_input_sensor?.find(
              o=>{
                return o.s_name == o_input_action__keep_hold_down_toggle_image_control?.s_name_input__controller
              }
            )
            let b_camera_control_layer = v_o_input_sensor_keep_hold_down_toggle_camera_control?.n_nor > 0.0;
            let b_image_control_layer = v_o_input_sensor_keep_hold_down_toggle_image_control?.n_nor > 0.0;
            if(b_camera_control_layer || b_image_control_layer){
              b_normal_layer = false;
            }
